    	...
    	while( have_rows('mainsection', $page->ID) ): the_row();
        	...
        	if(strlen(get_the_title($page->ID))>32) {$widget_area_name = $widget_area_name . ' ...';}
	      	$args = array(
						...
						'description'   => 'Section ' . $mainsection_index . ' : ' . get_sub_field('section_description'),
	        ...
	        register_sidebar( $args );
				...
				if( have_rows('overlay') ):
					...
					while( have_rows('overlay') ): the_row();
						if(get_sub_field('Overlay_type_selector') == 'widgetarea'):// Register Sidebars
							$widget_area_name = substr(get_the_title($page->ID),0,32);
							if(strlen(get_the_title($page->ID))>32) {$widget_area_name = $widget_area_name . ' ...';}
							$args = array(
								'id'            => 'widgetdynamicarea' . $page->ID . $mainsection_index . $overlaysection_index,
								'name'          => $widget_area_name,
								'description'   => 'Section ' . $mainsection_index . ' Overlay ' . $overlaysection_index . ': '. get_sub_field('overlay_description'),
								'before_title'  => '<div class="widget-title">',
								'after_title'   => '</div>',
								'before_widget' => '<div id="%1$s" class="overlaywidget %2$s">',
								'after_widget'  => '</div>',
							);
							register_sidebar( $args );
						endif;
